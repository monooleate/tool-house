<script lang="ts">
  ...
  const timing = getTimingConfig("png-avif");
  const INPUT_ACCEPT = "image/png,.png";
  const TOOL_SLUG = "png-avif";
  ...
  async function download() {
    ...
    if (results.length === 1) {
      ...
      await downloadZip(entries, "png-to-avif.zip");
    }
  }
</script>
...
{#if !files.length}
  <div class="dropzone-wrap">
    <Dropzone accept={INPUT_ACCEPT} multiple={true} maxSizeMB={50}
      label={ui.dragImageFor} sublabel="PNG -- Max. 50 MB" on:files={handleFiles} />
  </div>
{:else}
  <div class="add-more">
    <Dropzone accept={INPUT_ACCEPT} multiple={true} maxSizeMB={50}
