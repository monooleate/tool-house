...
<div class="tool-settings card">
  ...
  <p class="info-hint">A metadata torles ujrakodolja a kepet, igy minden EXIF, GPS es egyeb metaadat eltavolitasra kerul.</p>
</div>
...
{#if !file}
  <Dropzone
    ...
    multiple={false}
    maxSizeMB={50}
    label="Huzd ide a kepet a metadata torleshez"
...
{:else}
  <div class="preview-section">
    <div class="preview-grid">
      ...
      {#if resultUrl}
        <div class="preview-pane">
          <div class="preview-pane__label">Tisztitott{resultBlob ? ` -- ${formatFileSize(resultBlob.size)}` : ""}</div>
          <img src={resultUrl} alt="Tisztitott kep" class="preview-img" />
        </div>
      {/if}
    </div>
    ...
    <div class="actions">
      {#if !resultUrl}
        <button class="btn btn--primary" on:click={process} disabled={processing}>
          {processing ? "Feldolgozas..." : "Metadata torles"}
        </button>
      {:else}
