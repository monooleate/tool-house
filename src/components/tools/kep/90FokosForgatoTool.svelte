<script lang="ts">
  ...
  function setAngle(deg: number) {
    angle = deg;
    if (resultUrl) { URL.revokeObjectURL(resultUrl); resultUrl = ""; resultBlob = null; }
  }
...
{:else}
  <div class="preview-section">
    <div class="preview-grid">
      ...
      {#if resultUrl}
        <div class="preview-pane">
          <div class="preview-pane__label">{ui.rotated} ({angle}&#176;){resultBlob ? ` -- ${formatFileSize(resultBlob.size)}` : ""}</div>
    ...
    <div class="actions">
      {#if !resultUrl}
        <button class="btn btn--primary" on:click={process} disabled={processing}>
          {processing ? ui.processing : `${ui.rotate} ${angle}\u00B0`}
        </button>
      {:else}
