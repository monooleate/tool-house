<script lang="ts">
  ...
  function download() {
    if (resultBlob && file) {
      ...
      const name = file.name.replace(/\.[^.]+$/, "") + `-blur${blurRadius}.${ext}`;
...
<div class="tool-settings card">
  ...
  <div class="settings-row">
    <label class="label" for="blur-slider">Elmosas merteke: {blurRadius}px</label>
    <input id="blur-slider" type="range" min="1" max="20" step="1" bind:value={blurRadius} class="slider" />
  </div>
...
{#if !file}
  <Dropzone
    ...
    multiple={false}
    maxSizeMB={50}
    label="Huzd ide a kepet az elmosodashoz"
...
{:else}
  <div class="preview-section">
    <div class="preview-grid">
      ...
      {#if resultUrl}
        <div class="preview-pane">
          <div class="preview-pane__label">Elmosott ({blurRadius}px){resultBlob ? ` -- ${formatFileSize(resultBlob.size)}` : ""}</div>
          <img src={resultUrl} alt="Elmosott kep" class="preview-img" />
        </div>
      {/if}
    </div>
    ...
    <div class="actions">
      {#if !resultUrl}
        <button class="btn btn--primary" on:click={process} disabled={processing}>
          {processing ? "Feldolgozas..." : "Elmosas"}
        </button>
      {:else}
