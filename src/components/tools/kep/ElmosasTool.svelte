...
<div class="tool-settings card">
  ...
  <div class="settings-row">
    <label class="label" for="blur-slider">{ui.blur}: {blurRadius}px</label>
...
{:else}
  <div class="preview-section">
    <div class="preview-grid">
      ...
      {#if resultUrl}
        <div class="preview-pane">
          <div class="preview-pane__label">{ui.blur} ({blurRadius}px){resultBlob ? ` -- ${formatFileSize(resultBlob.size)}` : ""}</div>
          <img src={resultUrl} alt={ui.blurredImage} class="preview-img" />
        </div>
      {/if}
    </div>
    ...
    <div class="actions">
      {#if !resultUrl}
        <button class="btn btn--primary" on:click={process} disabled={processing}>
          {processing ? ui.processing : ui.blur}
        </button>
      {:else}
