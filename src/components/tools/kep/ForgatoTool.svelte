<script lang="ts">
  ...
  function download() {
    if (resultBlob && file) {
      ...
      const name = file.name.replace(/\.[^.]+$/, "") + `-rot${angle}.${ext}`;
...
<div class="tool-settings card">
  ...
  <div class="settings-row">
    ...
    <div class="quick-buttons">
      <button class="btn btn--sm" class:btn--primary={angle === 90} class:btn--outline={angle !== 90} on:click={() => setAngle(90)}>90&#176;</button>
      <button class="btn btn--sm" class:btn--primary={angle === 180} class:btn--outline={angle !== 180} on:click={() => setAngle(180)}>180&#176;</button>
      <button class="btn btn--sm" class:btn--primary={angle === 270} class:btn--outline={angle !== 270} on:click={() => setAngle(270)}>270&#176;</button>
    </div>
  </div>
...
.quick-buttons { display: flex; gap: var(--sp-2); margin-top: var(--sp-2); }
