...
<div class="tool-settings card">
  ...
  <p class="settings-hint">{ui.quality}: 90% (JPEG)</p>
</div>
...
{#if files.length > 0}
  <ConvertButton
    ...
    onConvert={convert}
    onDownload={download}
    convertLabel={"JPG " + (ui.conversion ?? "konvertálás")}
    downloadLabel={results.length > 1 ? (ui.zipDownload ?? "ZIP letöltés") : "JPG " + (ui.download ?? "letöltés")}
